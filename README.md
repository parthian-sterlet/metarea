...
- If they show the partial overlap, the joint FP rate increases an non-integer impact between one and two, depending on the ratios between the overlap length (OL), and the lebgth of motifs W1 and W2 (1 < dFP < 2, dFP = 0.5 * (OL / W1 + OL / W2) = 0.5 * OL * (W1 + W2) / (W1 * W2)).

The cases of full or partial overlaps impliy that joined hit possesses with the best estimate of ERR for two sites, -Log10(ERR,joined) = Max( -Log10(ERR1),  -Log10(ERR2)).
