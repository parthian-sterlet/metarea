...
int main(int argc, char* argv[])
{
	...
	char file_for[ARGLEN], file_back[ARGLEN], path_fasta[ARGLEN], pfile_for[ARGLEN], pfile_back[ARGLEN], partner_db[2][ARGLEN];
	char file_roc[ARGLEN], file_auc[ARGLEN], file_log[ARGLEN];
	FILE* outlog, * out_roc, * out_auc, * in_pwm[2];

	if (argc != 9)
	{
		printf("%s 1path_fasta 2foreground fasta 3background fasta 4,5binary file library1,2 6file ROC 7file pAUC 8file log", argv[0]);
		return -1;
	}
	...
	strcpy(partner_db[0], argv[4]); //h12hs, h12mm
	strcpy(partner_db[1], argv[5]); //h12hs, h12mm
	...
	for (n = 0; n < 2; n++)
	{
		...
		fread(&nthr_dist[n], sizeof(int), 1, in_pwm[n]);
	}
	double** thr_all;
	...
	for (i = 0; i < 2; i++)
	{
		thr_all[i] = new double[nthr_dist[i]];
	...
	for (i = 0; i < 2; i++)
	{
		fpr_all[i] = new double[nthr_dist[i]];
		if (fpr_all[i] == NULL) { fprintf(stderr, "Error: Out of memory..."); return -1; }
	}
	for (n = 0; n < 2; n++)
	{
		fread(thr_all[n], sizeof(double), nthr_dist[n], in_pwm[n]);
		fread(fpr_all[n], sizeof(double), nthr_dist[n], in_pwm[n]);
		fclose(in_pwm[n]);
	}
	...
	fclose(out_auc);
	if ((out_roc = fopen(file_roc, "at")) == NULL)
	{
